...
RUN apt-get update && apt-get install -qq -y --no-install-recommends \
      build-essential nodejs libpq-dev libmysqlclient-dev
...
ENV INSTALL_PATH /cabins
# The name of the application is cabins and while there
...
# having to CD into the /cabins folder every time would be
# lame, so instead we can set the WORKDIR to be /cabins.
#
...
# cabins/ path inside of the Docker image.
#
# It copies it to /cabins because of the WORKDIR being set.
#
...
# /cabins folder inside of the Docker image.
#
...
RUN bundle exec rake RAILS_ENV=production DATABASE_URL=mysql2://user:pass@127.0.0.1/dbname ACTION_CABLE_ALLOWED_REQUEST_ORIGINS=foo,bar SECRET_TOKEN=dummytoken assets:precompile
